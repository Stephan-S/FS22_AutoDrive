...
function FollowCombineTask:update(dt)
    ...
    if self.state == FollowCombineTask.STATE_CHASING then
        ...
        if AutoDrive.combineIsTurning(self.combine) then
            -- harvester turns
            --print("Waiting for turn now - 1- t:" ..  string(AutoDrive.combineIsTurning(self.combine)) .. " anglewrongtimer: " .. string(self.angleWrongTimer.elapsedTime > 10000))      
            AutoDrive.debugPrint(self.vehicle, AutoDrive.DC_COMBINEINFO, "Detected combine turning: " ..  string(AutoDrive.combineIsTurning(self.combine)) .. " - waiting for turn to be finished next")
